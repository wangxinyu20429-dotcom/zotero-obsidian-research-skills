#!/usr/bin/env python3
"""Search OpenAlex, check Zotero duplicates, and download lawful OA PDFs.
...
USER_AGENT = "zotero-hydrology-notes/0.3.1 OpenAlex-OA-client"
...
def search_command(args: argparse.Namespace) -> int:
    ...
    if run_path.exists():
        ...
        if previous.get("request_signature") == request_signature and candidates_path.exists() and duplicates_path.exists():
...
def build_parser() -> argparse.ArgumentParser:
    ...
    download.add_argument(
        ...
        action="append",
        required=True,
        help="Approved OpenAlex work ID; repeat for each item selected from the reviewed candidate list",
    )
